...
public class CAHolderAccelerateCreationProcessor : CAHolderTransactionProcessorBase<PreCrossChainSyncHolderInfoCreated>
{
    ...
    protected override async Task HandleEventAsync(PreCrossChainSyncHolderInfoCreated eventValue, LogEventContext context)
    {
        ...
        caHolderIndex = new CAHolderIndex
        {
            ...
            OriginChainId = ChainHelper.ConvertChainIdToBase58(eventValue.CreateChainId)
        };
        ObjectMapper.Map<LogEventContext, CAHolderIndex>(context, caHolderIndex);
        await CAHolderIndexRepository.AddOrUpdateAsync(caHolderIndex);
    }

    protected override async Task HandlerTransactionIndexAsync(PreCrossChainSyncHolderInfoCreated eventValue, LogEventContext context)
    {
